  ...
  render() {
    ...
    return (
      <div className={styles.detailBox} style={{ height: autoheight() + 'px' }}>
        ...
        <div className={styles.overY} style={{ height: autoheight() - 174 + 'px' ,background:btnName && btnName.length > 2 ? '#f9f9f9':'#fff'}}>
          <div className={styles.detail}>
            <div className={styles.detailText}>
              {params.xxmc.isvisible ?  <div className={styles.tilteMc}>{params.xxmc.msg}{params.xxzt.isvisible ? <span className={styles.status}>（{params.xxzt.msg}）</span> : ''}</div> : ''}
            </div>
          ...
          {btnName && btnName.length > 2 ?
            <div><div className={styles.appHeaders}>
              ...
              <TextArea rows={6} className={styles.textArea} placeholder="请在此输入内容" onChange={(e)=>this.getConnent(e)}/></div>:''}
              {/*{btnName && btnName.length > 2 ?*/}
        ...
        {btnName ? (
          <div className={styles.boxBtn}>
            <div className={styles.onOk} onClick={()=>this.getDb(cardTo)}>{btnName}</div>
          </div>
        ) : (
          ''
        )}
